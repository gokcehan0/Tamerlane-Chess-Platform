...
export default function Lobby() {
    ...
    const [timeControl, setTimeControl] = useState<number | null>(null); // null = unlimited, numbers in minutes
    ...
    async function handleCreateRoom() {
        ...
        setLoading(true);
        try {
            // Pass options to createRoom
            // Note: We need to update useOnlineGame to accept these!
            const options = {
                side: selectedSide,
                timeControl: timeControl ? timeControl * 60 : null // Store in seconds or null
            };
    ...
    async function handleJoinRoom(e: React.FormEvent) {
        ...
        try {
            ...
            // Security: Input Validation (NoSQL Injection Prevention)
            // Only allow alphanumeric characters to prevent path traversal or query injection
            if (!/^[A-Z0-9]+$/.test(code)) {
    ...
    return (
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
            <div style={{ marginBottom: '2rem' }}>
                <h1 style={{ fontSize: '2rem', color: colors.text, marginBottom: '0.5rem' }}>Game Lobby</h1>
                <p style={{ color: colors.textSec }}>Start a match or join an existing one.</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(290px, 1fr))', gap: '2rem' }}>
                ...
                <div style={{ background: colors.card, padding: '2rem', borderRadius: '12px', border: `1px solid ${colors.border}`, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                    ...
                    <h3 style={{ fontSize: '1.25rem', color: colors.text, marginBottom: '0.5rem' }}>Create Online Match</h3>
                    <p style={{ color: colors.textSec, marginBottom: '1.5rem' }}>Start a fresh game. You will get a room code to share.</p>
                ...
                <div style={{ background: colors.card, padding: '2rem', borderRadius: '12px', border: `1px solid ${colors.border}`, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                    ...
                    <p style={{ color: colors.textSec, marginBottom: '1.5rem' }}>Enter the unique room code shared by your friend.</p>
